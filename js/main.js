...
async function getsearchSelected(term) {
  ...
  for (let j = 0; j < allData.length; j++) {
    if (
      ...
      cartona += `<div class=" col-sm-6 col-md-6 col-lg-4">
...
function displayMoives() {
  ...
  for (let i = 0; i < allData.length; i++) {
    item += `<div class=" col-sm-6 col-md-6 col-lg-4">
        <div class="content rounded-3">
            <img src="https://image.tmdb.org/t/p/w500/${
...
$(".openCloseNav").click(function () {
  let navSize = $("#navbar").innerWidth();

  if ($("#navbar").css("left") == `${-navSize}px`) {
        $("#navbar").animate({ left: 0 }, 700, function () {
          $(".openCloseNav").animate({left: `15%`}, 250, function () {
            
          $(".navLinks li").eq(0).animate({ paddingTop: "0", opacity: 1 }, 200, function () {
              $(".navLinks li")
                .eq(1)
        ...
        $("#navbar").animate({ left: -navSize }, 700, function () {
          $(".openCloseNav").animate({left: '100%' }, 250, function () {
...
function validation() {
  ...
  if (regPassword.test(userPassword.value)) {
    ...
    userPassword.placeholder = "password must contain a special and capital character";
  }
  if (
